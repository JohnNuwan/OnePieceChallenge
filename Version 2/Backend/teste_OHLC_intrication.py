	...
	for i in df['Name']:
		for j in Forex:
			if j == i:
				...
				data = get_data_trade(j,60)
				print(data)
				time_now = datetime.now()


				if data["Tradable"][-1] == 1 or data["Tradable"][-1] == -1:
					data = get_data_trade(j,30)
					print(data)
					MESSAGE = f"symbol : {j} ,\n Zone Tradable \n Price : {data['close'][-1]} \n UT : {30} Minute"
